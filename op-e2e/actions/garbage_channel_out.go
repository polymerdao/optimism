...
type ChannelOutIface interface {
	ID() derive.ChannelID
	Reset() error
	AddBlock(block eth.EthBlock) (uint64, error)
	ReadyBytes() int
	Flush() error
	Close() error
...
// should be made.
func (co *GarbageChannelOut) AddBlock(block eth.EthBlock) (uint64, error) {
	if co.closed {
...
func blockToBatch(block eth.EthBlock) (*derive.BatchData, error) {
